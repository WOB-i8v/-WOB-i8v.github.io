  ...
  try {
    ...
    if (vd) {
      ...
      gc ? drawTutorialMobile(k) : Kc + 8E3 > a && !uc && df();
        ...
        Ad + 9500 > a && ef();
        // HACK: compact in-game overlay (top-left, below scrap bar - no overlaps)
        if (vd) {
          k.save();
          // FPS counter — y=90, below the scrap bar image (which ends ~y=84)
          if (hackFPS) {
          ...
          if (hackThreatMeter) {
            ...
            k.globalAlpha = 0.82;
            k.fillText("THREAT: " + hackTCount, 8, 104);
          }
          // Active-mod pill list — y=116+, only ON mods shown, faint so it doesn't distract
          var hackActiveList = [];
          if (hackAutoFire)    hackActiveList.push("AUTOFIRE");
          if (hackAutoAim)     hackActiveList.push("AUTOAIM");
          if (hackTracers)     hackActiveList.push("TRACERS");
          if (hackAutoScrap)   hackActiveList.push("SCRAP");
          if (hackESPArrows)   hackActiveList.push("ESP");
          if (hackPredictLine) hackActiveList.push("PREDICT");
          if (hackEnemyDist)   hackActiveList.push("DIST");
          if (hackHPNumbers)   hackActiveList.push("HP#");
          if (hackEnemyRadar)  hackActiveList.push("RADAR");
          if (hackRangeRing)   hackActiveList.push("RING");
          if (hackActiveList.length > 0) {
            k.font = "9px xirod";
            k.fillStyle = "#77EEFF";
            k.globalAlpha = 0.38;
            var hackListY = (hackFPS ? 1 : 0) + (hackThreatMeter ? 1 : 0);
            hackListY = 116 + hackListY * 0; // always start at 116
            for (var hi = 0; hi < hackActiveList.length; hi++) {
              k.fillText("● " + hackActiveList[hi], 8, 116 + hi * 11);
            }
          }
          k.globalAlpha = 1;
          k.restore();
...
// ============================================================
// HACK: Settings UI - injected at window.onload
// ============================================================
function hackInitSettingsUI() {
  // ── Load persisted settings ────────────────────────────────
  try {
    var saved = JSON.parse(localStorage.hackModSettings || '{}');
    var bools = ['hackAutoFire','hackTracers','hackAutoScrap','hackHPNumbers',
                 'hackEnemyRadar','hackRangeRing','hackAutoAim','hackESPArrows',
                 'hackPredictLine','hackThreatMeter','hackFPS','hackEnemyDist'];
    for (var bi = 0; bi < bools.length; bi++) {
      if (bools[bi] in saved) window[bools[bi]] = saved[bools[bi]];
    }
  ...
  function hackSave() {
    try {
      localStorage.hackModSettings = JSON.stringify({
        hackAutoFire:hackAutoFire, hackTracers:hackTracers,
        hackAutoScrap:hackAutoScrap, hackHPNumbers:hackHPNumbers,
        hackEnemyRadar:hackEnemyRadar, hackRangeRing:hackRangeRing,
        hackAutoAim:hackAutoAim, hackESPArrows:hackESPArrows,
        hackPredictLine:hackPredictLine, hackThreatMeter:hackThreatMeter,
        hackFPS:hackFPS, hackEnemyDist:hackEnemyDist,
        hackFireRate:hackFireRate, hackZoom:hackZoom
      });
    } catch(e) {}
  }
  ...
  css.textContent =
    '#hackBtn{margin-top:10px;background:transparent;border:2px solid #77eeff;' +
    'color:#77eeff;font-family:xirod,monospace;font-size:12px;padding:7px 0;' +
    'cursor:pointer;letter-spacing:2px;display:block;width:100%;' +
    'transition:background .15s,color .15s}' +
    '#hackBtn:hover{background:#77eeff;color:#000}' +
    '#hackOverlay{display:none;position:fixed;inset:0;background:rgba(0,0,10,.92);' +
    'z-index:9999;align-items:center;justify-content:center}' +
    '#hackPanel{background:#05050f;border:1px solid #77eeff;' +
    'box-shadow:0 0 50px rgba(119,238,255,.2),inset 0 0 30px rgba(0,0,20,.5);' +
    'padding:26px 22px;width:460px;max-width:93vw;max-height:88vh;overflow-y:auto;' +
    'font-family:xirod,monospace;color:#bbb;' +
    'scrollbar-width:thin;scrollbar-color:#77eeff #080816}' +
    '#hackPanel h2{color:#77eeff;text-align:center;margin:0 0 2px;font-size:17px;' +
    'letter-spacing:5px;text-shadow:0 0 14px rgba(119,238,255,.55)}' +
    '#hackPanel .hkSub{text-align:center;color:#2a3a50;font-size:9px;' +
    'letter-spacing:3px;margin:0 0 18px}' +
    '.hkSec{font-size:9px;color:#2a3a50;letter-spacing:3px;' +
    'margin:14px 0 5px;border-bottom:1px solid #0c0c1e;padding-bottom:3px}' +
    '.hkRow{display:flex;align-items:center;gap:10px;' +
    'padding:6px 0;border-bottom:1px solid #09091a}' +
    '.hkInfo{flex:1;min-width:0}' +
    '.hkName{font-size:11px;letter-spacing:1px;color:#ccc}' +
    '.hkDesc{font-size:9px;color:#334;margin-top:2px;line-height:1.3}' +
    '.hkKey{font-size:9px;color:#465e84;font-family:monospace;' +
    'background:#08081a;border:1px solid #2a3a50;padding:2px 6px;' +
    'white-space:nowrap;flex-shrink:0}' +
    '.hkTog{position:relative;width:42px;height:22px;flex-shrink:0}' +
    '.hkTog input{opacity:0;width:0;height:0;position:absolute}' +
    '.hkSlid{position:absolute;cursor:pointer;inset:0;' +
    'background:#08081a;border:1px solid #1e2840;border-radius:11px;' +
    'transition:border-color .2s}' +
    '.hkSlid:before{content:"";position:absolute;width:16px;height:16px;' +
    'left:2px;top:2px;background:#1e2840;border-radius:50%;' +
    'transition:transform .2s,background .2s,box-shadow .2s}' +
    'input:checked+.hkSlid{border-color:#77eeff}' +
    'input:checked+.hkSlid:before{transform:translateX(20px);' +
    'background:#77eeff;box-shadow:0 0 8px rgba(119,238,255,.7)}' +
    '.hkSlRow{display:flex;align-items:center;gap:10px;' +
    'padding:8px 0;border-bottom:1px solid #09091a}' +
    '.hkSlRow input[type=range]{flex:1;-webkit-appearance:none;height:3px;' +
    'background:#08081a;border:1px solid #1e2840;outline:none;border-radius:2px}' +
    '.hkSlRow input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;' +
    'width:13px;height:13px;background:#77eeff;border-radius:50%;cursor:pointer;' +
    'box-shadow:0 0 5px rgba(119,238,255,.6)}' +
    '.hkVal{font-size:10px;color:#77eeff;min-width:38px;text-align:right;flex-shrink:0}' +
    '#hackClose{display:block;width:100%;margin-top:18px;background:transparent;' +
    'border:1px solid #2a3a50;color:#465e84;font-family:xirod,monospace;' +
    'font-size:11px;padding:9px;cursor:pointer;letter-spacing:3px;' +
    'transition:border-color .15s,color .15s}' +
    '#hackClose:hover{border-color:#77eeff;color:#77eeff}';
  document.head.appendChild(css);

  // ── Modal HTML ─────────────────────────────────────────────
  function row(id, name, desc, key) {
    return '<div class="hkRow">' +
      '<div class="hkInfo"><div class="hkName">'+name+'</div>' +
      '<div class="hkDesc">'+desc+'</div></div>' +
  ...
  overlay.innerHTML =
    '<div id="hackPanel">' +
      ...
      row('hk_autoaim',  'AUTO AIM',  'Continuously rotates aim toward nearest enemy',   'T') +
      sliderRow('hk_firerate','hk_fv', 'FIRE RATE', 'Milliseconds between shots &mdash; lower is faster (vanilla: 100ms)', 40, 200, 5) +

      '<div class="hkSec">VISUAL</div>' +
      row('hk_hpnum',    'HP NUMBERS',      'Exact health values displayed beside health bars',          '') +
      ...
      row('hk_predict',  'PREDICTION LINES', 'Dashed trajectory showing where each ship is heading',     'N') +
      row('hk_dist',     'ENEMY DISTANCE',   'Distance in world units printed next to each player name', 'D') +
      sliderRow('hk_zoom','hk_zv', 'ZOOM', 'Camera zoom level &mdash; also adjustable with scroll wheel in-game', 0.3, 3.5, 0.1) +

      '<div class="hkSec">RADAR &amp; AWARENESS</div>' +
      row('hk_radar', 'ENEMY RADAR',   'Shows all enemies as dots on the minimap',                          '') +
      row('hk_esp',   'ESP ARROWS',    'Edge-of-screen arrows pointing to off-screen players, with HP bars', 'H') +
      row('hk_threat','THREAT METER',  'HUD counter showing how many enemies are near you',                  '') +

      '<div class="hkSec">UTILITY</div>' +
      row('hk_scrap', 'AUTO SCRAP', 'Steers your ship toward the nearest scrap pickup', '\\') +
      row('hk_fps',   'FPS COUNTER','Frames-per-second display in the top-left corner',  'Z') +
  ...
  function tog(id, get, set) {
    ...
    el.checked = get();
    el.onchange = function() { set(this.checked); hackSave(); };
  }
  function sld(slId, vlId, get, set, fmt) {
    var sl = document.getElementById(slId), vl = document.getElementById(vlId);
    sl.value = get(); vl.textContent = fmt(get());
    sl.oninput = function() { var v = parseFloat(this.value); set(v); vl.textContent = fmt(v); hackSave(); };
  }

  tog('hk_autofire',  function(){return hackAutoFire;},    function(v){hackAutoFire=v;});
  tog('hk_autoaim',   function(){return hackAutoAim;},     function(v){hackAutoAim=v;});
  tog('hk_hpnum',     function(){return hackHPNumbers;},   function(v){hackHPNumbers=v;});
  tog('hk_rangering', function(){return hackRangeRing;},   function(v){hackRangeRing=v;});
  tog('hk_tracers',   function(){return hackTracers;},     function(v){hackTracers=v;});
  tog('hk_predict',   function(){return hackPredictLine;}, function(v){hackPredictLine=v;});
  tog('hk_dist',      function(){return hackEnemyDist;},   function(v){hackEnemyDist=v;});
  tog('hk_radar',     function(){return hackEnemyRadar;},  function(v){hackEnemyRadar=v;});
  tog('hk_esp',       function(){return hackESPArrows;},   function(v){hackESPArrows=v;});
  tog('hk_threat',    function(){return hackThreatMeter;}, function(v){hackThreatMeter=v;});
  tog('hk_scrap',     function(){return hackAutoScrap;},   function(v){hackAutoScrap=v;});
  tog('hk_fps',       function(){return hackFPS;},         function(v){hackFPS=v;});

  sld('hk_firerate','hk_fv',
    function(){return hackFireRate;},
    function(v){hackFireRate=v;},
    function(v){return v+'ms';});
  sld('hk_zoom','hk_zv',
    function(){return hackZoom;},
    function(v){hackZoom=v; if(typeof Ke!='undefined') Ke();},
    function(v){return v.toFixed(1)+'x';});

  // ── Open / close ────────────────────────────────────────────
  function hackOpenPanel() {
    // Re-sync UI to current live values before showing
    tog('hk_autofire',  function(){return hackAutoFire;},    function(v){hackAutoFire=v;});
    tog('hk_autoaim',   function(){return hackAutoAim;},     function(v){hackAutoAim=v;});
    tog('hk_hpnum',     function(){return hackHPNumbers;},   function(v){hackHPNumbers=v;});
    tog('hk_rangering', function(){return hackRangeRing;},   function(v){hackRangeRing=v;});
  ...
  document.getElementById('hackClose').onclick = function() { overlay.style.display='none'; };
  overlay.onclick = function(ev) { if(ev.target===overlay) overlay.style.display='none'; };

  // ── Button injected into the pregame screen ─────────────────
  var btn = document.createElement('button');
  btn.id = 'hackBtn';
  btn.textContent = '\u2699  MOD SETTINGS';
  btn.onclick = hackOpenPanel;
  document.getElementById('pregame').appendChild(btn);
}
